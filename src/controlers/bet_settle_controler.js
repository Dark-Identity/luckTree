const {User , Bet , Lottery , Deposit , Withdrawal} = require('../db');
...
class admin_function{
  ...
  static settle_bet = async(req, res)=>{
    ...
    if(!id || id == 'undefined'){
      ...
      let lottery_cnt = await Lottery.find({FINISHED : 0 , LOTTERY_ID : id}).count();
      
      if(lottery_cnt < 1){
        return res.send({status : `only ${lottery_cnt} people purchased.`});
      }else{
         
        let user_inv_arr = await Lottery.find({LOTTERY_ID : id , FINISHED : 0} , 
          {_id : 0 , INVITATION_CODE : 1 , AMOUNT : 1});

         console.log(user_inv_arr);
         
         let random_pos = Math.floor(Math.random()*user_inv_arr.length);
         let winner_inv = parseInt(user_inv_arr[random_pos]['INVITATION_CODE']);
         let amount = parseFloat(user_inv_arr[random_pos]['AMOUNT']);
        
        //  we got our winner update it saperately
        await Lottery.findOneAndUpdate({ INV : winner_inv } , 
           {
            RESULT : 'WINNER',
            FINISHED : 1 
          }) 
        // Giving the winner full amount
        await User.findOneAndUpdate({INV : winner_inv} , {
          $inc : {
            BALANCE : 10*amount,
            W_AMOUNT : 10*amount
          }
...
console.log(path.join( __dirname , '../' , '../' , 'publc/WINNERS' ))
